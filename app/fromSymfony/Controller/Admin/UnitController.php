...
class UnitController extends Controller
{
    ...
    public function postAction(Request $request)
    {
      ...
      if ($unitData['_id']) {
        ...
        // generate UI if empty
        if ($unitData['view'] == '') {
            $hookupAsArray = Yaml::parse($unitData['hookup']);
            $uim = new UIMapper($hookupAsArray);
            $unitData['view'] = $uim->getView();
        }
        // simple fields
        ...
        try {
          $hookupAsArray = Yaml::parse($unitData['hookup']);
          $unitData['hookup'] = serialize($hookupAsArray);
